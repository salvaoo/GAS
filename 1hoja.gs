...
function onOpen() {
  ...
  .addItem("Identificar Duplicados", "abrimenu")
...
function TodaLaHoja() {
  ...
  var newData = [];
  /*
   *  Recorremos todas las filas de "data", 
   *  para cada fila comparamos con todas las filas de newData, 
   *  si la fila actual ya existe en newData no la incorporamos
   */
  for (i in data) {
    //Fila actual
    var row = data[i];
...
/*
 *  Funcion que permite seleccionar las columnas que
 *  se tomarán como referencia para establecer se una fila está duplicada.
 */
function seleccionColumnas() {
  ...
  */
  ...
  //variable numeroColumnas va a contener las columnas seleccionadas en valores numericos.
  var numeroColumnas = [];
  /*  
  ...
  for (var cont = 0; cont < columnas.length; cont++) {
    ...
    var resta = parseInt(numCol) - 64;   //Aqui sacamos el numero de columna correspondiente a la letra de las columnas seleccionadas.
  ...
  for (i in data) {
    ...
    for (var j=0; j < newData.length && !duplicados;j++) {   //Entramos si es menor j que el valor de newData y duplicados es igual a false.
...
/****************************
 ...
 Con esta funcion podemos eliminar aquellas filas de la hoja de trabajo que estén presente 
 en una segunda hoja. Los valores de referencia(columnas) para determinar si una fila es duplicada o no
 , se tomarán de la selección previa en la propia hoja.

****************************/

/*
   Funcion de control que solicita el input del usuario, comprueba las columnas seleccionadas
   y ejecuta la funcion para eliminar duplicados
*/
   ...
   if (!hojaB){ // Validación de la hojaB, si no existe, abortamos
     ...
     */
     seleccion.forEach(function(e){ // ej. seleccion -> [A1:A5, C2]
       var rango = e.split(',');
       rango.forEach(function(elem){ //Para cada rango (en caso de selección múltiple
                                     //no consecutiva ej. rango[0]-> A1:A5
         elem.split(':').forEach(function(element){ // Para cada celda (ColumnaFila), miramos 
                                                    //qué letra(columna) es ej. elem[0]-> A1, elem[1]->A5
           var col = element.charCodeAt(0)-64; // Sacamos de elem[0] el primer caracter (la columna) 
                                               //y la convertimos en su nº de columna  ej. col = A 
           if(columnasDeA.indexOf(col) < 0){ // Si en nuestro array de columnas, 
                                             //no tenemos ya esta columna, la agregamos al array.
...
function eliminarDuplicados2Hojas(hojaB, columnasDeA, hojaA, nFilasEncabezado){
  ...
  dataB = hojaB.getDataRange().getValues().map(function (e){ //En este caso sólo incorporamos
                                                             //las columnas implicadas en la selección
  ...
  */
  ...
  /*
     Recorremos todas las filas en nuestra tablaA, y para cada una de ellas las comparamos
     con todas las filas de la tablaB, si existe alguna coincidencia, significa que está duplicada
     en la segunda tabla y no nos la traemos a la nueva tabla.
  */
